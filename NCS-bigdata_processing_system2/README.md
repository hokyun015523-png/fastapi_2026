...
## 대구 교통 데이터 처리시스템 예제
저장시스템 실습에서 만든 PostgreSQL 테이블을 이용하여 배치처리, 실시간 처리, 이벤트 처리 모듈을 실행하는 예제

### 1. 전제 조건
지난 주 저장 시스템 시습이 완료되어 있엉야 한다.
...
### 2. 필요한 라이브러리
```bash
uv add sqlchemy psycopg2-binary
```

### 3. 환경변수
기본값은 저장시스템 실습과 같은 비밀번호 `1234` 사용
...
```sql
SELECT * FROM traffic_subway_hourly_summary ORDER BY total_passemgers DESC LIMIT 20;
SELECT * FROM traffic_realtime_window_summary ORDER BY processed_at DESC LIMIT 20;
SELECT * FROM traffic_subway_event_alerts  ORDER BY detcted_at DESC LIMIT 20;
```
